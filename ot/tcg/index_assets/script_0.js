        ...
        async function resetDefaults() {
            if (!confirm("ACHTUNG: Dies löscht ALLE Kategorien und Kriterien und lädt die Standard-Daten neu!")) return;
        ...
        function renderHumans() {
            ...
            if (pageItems.length === 0) {
                list.innerHTML = `<div class="col-span-full text-center py-20 text-slate-400 font-bold flex flex-col items-center gap-2"><span class="text-4xl">🔍</span><br>Keine Personen gefunden.</div>`;
                return;
            }
                ...
                list.innerHTML += `
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-slate-300 transition group flex flex-col h-full hover:shadow-lg relative overflow-hidden">
                        ...
                        <div class="flex-grow relative z-10">
                            <p class="text-xs text-slate-400 font-bold flex items-center gap-1"><span class="text-slate-300">●</span> ${h.ratingCount || 0} Bewertungen</p>
                        </div>
        ...
        function openRate(id) {
            ...
            if (myCriteria.length === 0) {
                container.innerHTML = '<p class="text-center text-slate-400 py-4">Keine Kriterien für diese Person definiert.</p>';
            }

            myCriteria.forEach(crit => {
                // Radio buttons 0-6
                let radios = `<label class="cursor-pointer mr-3 opacity-60 hover:opacity-100 transition group flex flex-col items-center"><input type="radio" name="${crit.id}" value="0" class="sr-only peer" checked><span class="block w-9 h-9 rounded-lg bg-slate-200 text-slate-500 font-bold flex items-center justify-center peer-checked:bg-slate-300 peer-checked:ring-2 peer-checked:ring-slate-300">✕</span></label>`;
        ...
        function renderAdminHumans() {
            ...
            const humansList = humansData.map(h => {
                ...
                <div class="flex justify-between items-center bg-white p-4 border rounded-xl hover:border-indigo-300 transition group shadow-sm">
                    ...
                    <div class="flex gap-2 opacity-100 sm:opacity-60 group-hover:opacity-100 transition">
                        ...
                        <button onclick="deleteHuman('${h._id}')" class="text-red-500 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-bold">Löschen</button>
                    </div>
                </div>`;
            }).join('');
            ...
            c.innerHTML = `
                <div class="lg:col-span-1 bg-slate-50 p-5 rounded-xl h-fit border border-slate-200 sticky top-0">
                    <h3 class="font-bold mb-4 text-slate-700" id="form-title">Person hinzufügen</h3>
                    ...
                    <div class="mb-4"><label class="block text-xs font-bold text-slate-400 uppercase mb-1">Kategorie</label><select id="new-human-cat" class="w-full p-3 border rounded-lg bg-white" onchange="renderNewHumanCriteria()"><option value="">Bitte wählen...</option>${options}</select></div>
                    <div class="mb-2 font-bold text-xs text-slate-400 uppercase tracking-wide">Kriterien:</div>
                    <div id="criteria-select" class="max-h-48 overflow-y-auto mb-4 bg-white p-3 rounded-lg border border-slate-200 text-sm space-y-1 shadow-inner"><div class="text-slate-400 italic text-center py-2">Erst Kategorie wählen</div></div>
                    <div class="flex gap-2">
                        ...
                        <button onclick="resetHumanForm()" id="btn-cancel-human" class="hidden px-3 py-3 rounded-lg font-bold text-slate-500 hover:bg-slate-200 transition">✕</button>
                    </div>
                </div>
        ...
        function renderNewHumanCriteria(preIds = []) {
            ...
            if (!cat) { div.innerHTML = '<div class="text-slate-400 italic text-center py-2">Erst Kategorie wählen</div>'; return; }
        ...
        function resetHumanForm() {
            ...
            document.getElementById('criteria-select').innerHTML = '<div class="text-slate-400 italic text-center py-2">Erst Kategorie wählen</div>';
            document.getElementById('form-title').innerText = "Person hinzufügen";
        ...
        async function showUserRatings(uid, uname) {
            ...
            try {
                ...
                if (res.ok) {
                    const data = await res.json();
                    let html = `<button onclick="renderAdminModeration()" class="mb-4 text-indigo-600 font-bold flex items-center gap-1 hover:underline">&larr; Zurück</button><h3 class="text-xl font-bold mb-6 border-b pb-2">Bewertungen von <span class="text-indigo-600">${uname}</span></h3><div class="space-y-3 max-h-[600px] overflow-y-auto">`;
        ...
        function renderCert(data, title, isGlobal) {
            ...
            let yContentEnd = 290; // Start Y für Inhalt
            ...
            if (isTeacher) {
                ...
                // Hauptfächer (2 Spalten -> halbe Höhe)
            ...
            // Gesamthöhe bestimmen (Footer Position + Platz für Text & Rand unten)
            // A4 Standard ist 842px. Wir verlängern, wenn nötig.
            ...
            drawText("für", w/2, 200, 16, "serif", "center", "#475569", "italic");
            ...
            if (isTeacher) {
                ...
                if (secCrits.length > 0) {
                    drawText("Wahlpflichtfächer / Weitere", 70, ySec, 12, "sans", "left", "#64748b", "bold italic");
                    ySec += 30;
            ...
            // yFooter + 60 ist sicher unter den Unterschriften und über h-40 (Rahmen unten)
