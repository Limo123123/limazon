        ...
        const WHEELS_API_URL = 'https://api.limazon.v6.rocks/api';
        ...
        function updateWheelLoginStatusUI() { const c = document.getElementById('wheel-customer-name'), tD = document.getElementById('wheel-user-tokens-display'), t = document.getElementById('wheel-user-tokens'), aB = document.getElementById('wheel-auth-buttons'), uA = document.getElementById('wheel-user-actions'), mC = document.getElementById('main-content-wheels'), lP = document.getElementById('login-prompt-wheels'); if (wheelLoggedInUser) { c && (c.textContent = wheelLoggedInUser.username); t && (t.textContent = wheelLoggedInUser.tokens || 0); tD && tD.classList.remove('hidden'); aB && aB.classList.add('hidden'); uA && uA.classList.remove('hidden'); mC && mC.classList.remove('hidden'); lP && lP.classList.add('hidden') } else { c && (c.textContent = 'Gast'); tD && tD.classList.add('hidden'); aB && aB.classList.remove('hidden'); uA && uA.classList.add('hidden'); mC && mC.classList.add('hidden'); lP && lP.classList.remove('hidden') } }
        ...
        const MAX_SEGMENTS = 50, MIN_SEGMENTS = 2;
        function resetAndShowWheelForm(d = null) { currentEditWheelId = d?._id; document.getElementById('wheel-form-title').textContent = d ? 'Rad bearbeiten' : 'Neues Rad'; document.getElementById('wheel-edit-id').value = d?._id || ''; document.getElementById('wheel-name').value = d?.name || ''; document.getElementById('wheel-description').value = d?.description || ''; document.getElementById('wheel-spin-cost').value = d?.spinCost ?? 1; document.getElementById('wheel-creation-cost').value = d?.creationCostPaid ?? 0; document.getElementById('wheel-is-public').checked = d ? (d.isPublic ?? true) : false; const e = document.getElementById('wheel-segments-editor'); e.innerHTML = ''; (d?.segments?.length >= MIN_SEGMENTS ? d.segments : [{ text: 'Gewinn A', color: '#FFD700' }, { text: 'Niete', color: '#C0C0C0' }]).forEach(s => addWheelSegmentField(s.text, s.color)); document.getElementById('wheel-form-message').textContent = '' }
        async function loadWheelForEditing(wheelId) {
            ...
            } catch (error) { showWheelsToast(`Fehler Laden des Rads: ${error.message}`, true); showView('my'); }
        }
        ...
        async function saveWheel() {
            if (!wheelLoggedInUser) { showWheelsToast("Bitte zuerst einloggen.", true); openAuthModal('login'); return; }
            const msgEl = document.getElementById('wheel-form-message'); msgEl.textContent = "Speichere..."; msgEl.className = 'text-sm mt-3 text-blue-500';
            const name = document.getElementById('wheel-name').value.trim();
            const description = document.getElementById('wheel-description').value.trim();
            const spinCost = parseInt(document.getElementById('wheel-spin-cost').value, 10);
            const isPublic = document.getElementById('wheel-is-public').checked;
            const segments = [];
            document.querySelectorAll('#wheel-segments-editor .wheel-segment-editor').forEach(div => { const txtIn = div.querySelector('input[type="text"]'), colIn = div.querySelector('input[type="color"]'); if (txtIn && colIn && txtIn.value.trim()) segments.push({ text: txtIn.value.trim(), color: colIn.value }) });
            if (!name || name.length < 3) { msgEl.textContent = "Name mind. 3 Zeichen."; msgEl.className = 'text-sm mt-3 text-red-500'; return; }
            if (isNaN(spinCost) || spinCost < 0) { msgEl.textContent = "Drehkosten ungültig."; msgEl.className = 'text-sm mt-3 text-red-500'; return; }
            if (segments.length < MIN_SEGMENTS) { msgEl.textContent = `Mindestens ${MIN_SEGMENTS} Segmente erforderlich.`; msgEl.className = 'text-sm mt-3 text-red-500'; return; }
            const wheelDataPayload = { name, description, isPublic, segments, spinCost };
            let url, method; const editId = document.getElementById('wheel-edit-id').value;
            if (editId) { method = 'PUT'; url = `${WHEELS_API_URL}/wheels/${editId}` } else { method = 'POST'; url = `${WHEELS_API_URL}/wheels`; const creationCost = parseInt(document.getElementById('wheel-creation-cost').value, 10); if (isNaN(creationCost) || creationCost < 0) { msgEl.textContent = "Erstellungskosten ungültig."; msgEl.className = 'text-sm mt-3 text-red-500'; return; } wheelDataPayload.creationCost = creationCost }
            try { const response = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(wheelDataPayload), credentials: 'include' }); const responseText = await response.text(); if (!response.ok) { let errorMessage = `Fehler (${response.status})`; if (responseText) { try { const eJ = JSON.parse(responseText); errorMessage = eJ.error || responseText.substring(0, 50) } catch (e) { errorMessage = responseText.substring(0, 50) } } throw new Error(errorMessage) } let result = {}; try { result = JSON.parse(responseText) } catch (e) { } showWheelsToast(result.message || (method === 'PUT' ? "Gespeichert!" : "Erstellt!"), false); if (result.user && result.user.tokens !== undefined) { wheelLoggedInUser.tokens = result.user.tokens; updateWheelLoginStatusUI() } document.getElementById('wheel-edit-id').value = ''; resetAndShowWheelForm(); showView('my') } catch (error) { msgEl.textContent = `Fehler: ${error.message}`; msgEl.className = 'text-sm mt-3 text-red-500' }
        }
        ...
        function openWheelSpinModal(wheelData) {
            ...
            if (wheelSpinning && theWheel) theWheel.stopAnimation(false);
            wheelSpinning = false;

            // TEXT LOGIK VERBESSERT
            const segmentsForWinwheel = currentOpenWheelData.segments.map(s => {
                let displayText = s.text;
                // Härteres Kürzen bei sehr langem Text
                if (displayText && displayText.length > 15) displayText = displayText.substring(0, 12) + "...";
                return { fillStyle: s.color, text: displayText, textFillStyle: determineTextColor(s.color) };
            });

            let numSegments = segmentsForWinwheel.length;
            // Dynamische Schriftgröße und Margin um Textüberlauf zu verhindern
            let fSize = 14, tMargin = 20, align = 'center';
            if (numSegments >= 8) { fSize = 10; align = 'outer'; }
            if (numSegments >= 12) { fSize = 8; tMargin = 15; }
            if (numSegments >= 20) { fSize = 7; tMargin = 10; }
            ...
            theWheel = new Winwheel({
                canvasId: 'wheelCanvas', numSegments: numSegments, outerRadius: 150, innerRadius: 0,
                textFontSize: fSize, textFontFamily: 'Roboto, Arial, sans-serif', textOrientation: 'curved',
                textAlignment: align, textMargin: tMargin,
                segments: segmentsForWinwheel,
                pointerAngle: 0, lineWidth: 1, strokeStyle: '#444444',
                animation: { type: 'spinToStop', duration: 8, spins: 10, callbackFinished: displaySpinResult },
                pins: { number: numSegments * 2, fillStyle: 'silver', outerRadius: 4, responsive: true },
                responsive: true
            });
        ...
        function closeWheelSpinModal() {
            // 1. Modal ausblenden (Das triggert den Observer)
            document.getElementById('wheel-spin-modal').classList.add('hidden');
            
            // 2. Cleanup (Animation stoppen, Variablen resetten)
            // Wir machen das hier direkt, damit es sich sofort "richtig" anfühlt
            if (theWheel) {
                theWheel.stopAnimation(false);
                theWheel.rotationAngle = 0;
                theWheel.draw();
                theWheel = null; // Wichtig für Reset beim nächsten Start
            }
            wheelSpinning = false;
        ...
        async function spinTheWheel() {
            if (!wheelLoggedInUser || !currentOpenWheelData || wheelSpinning || !theWheel) return;
            if (wheelLoggedInUser.tokens < currentOpenWheelData.spinCost) { showWheelsToast(`Zu wenig Tokens (${currentOpenWheelData.spinCost})`, true); return }
            
            // FIX: Rad komplett zurücksetzen vor jedem Spin!
            theWheel.stopAnimation(false);
            theWheel.rotationAngle = 0;
            theWheel.draw();
            ...
            document.getElementById('wheel-spin-result-message').textContent = "Drehe...";
            ...
            try {
                ...
                const winSegNum = result.winningSegmentIndex + 1;
                const segDet = theWheel.segments[winSegNum];
                let stopAt = segDet.startAngle + ((segDet.endAngle - segDet.startAngle) / 2);
                
                // Berechnung des Ziels
                stopAt = (stopAt + 360) % 360; 
            ...
            const observer = new MutationObserver(function (mutationsList) {
                for (const mutation of mutationsList) {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                        // Wenn das Modal versteckt wurde (durch X oder Code)
                        if (mutation.target.classList.contains('hidden')) {
                            console.log("Observer: Modal ist zu. Führe Cleanup durch.");
                            
                            // HIER KEIN closeWheelSpinModal() AUFRUFEN!
                            // Nur interne Variablen zurücksetzen (Sicherheitsnetz)
